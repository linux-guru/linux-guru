...
if ( ! class_exists( 'WPSC_Upgrade_DB_V2' ) ) :
	...
	final class WPSC_Upgrade_DB_V2 {
		...
		public static function import_thankyou_settings() {
			...
			$thank_you_text = wpsc_upgrade_macros( get_option( 'wpsc_thankyou_html' ) );
			$thank_page = get_option( 'wpsc-gs-thankyou-page-settings' );
			$thank_page['thankyou-html'] = $thank_you_text;
			$thank_page['thank-you-page-url'] = get_option( 'wpsc_thankyou_url' );
			$string_translations['wpsc-thankyou-html'] = $thank_you_text;

			update_option( 'wpsc-string-translation', $string_translations );
			update_option( 'wpsc-gs-thankyou-page-settings', $thank_page );
		}
					...
					switch ( intval( $type ) ) {
						...
						case 10:
							...
							foreach ( $prev_val as $item ) {
								...
								$filepath = $upload_dir['basedir'] . '/wpsc/' . $term_meta['save_file_name'][0];
								if ( isset( $term_meta['is_restructured'] ) && $term_meta['is_restructured'][0] ) {
									...
									$filepath = $upload_dir['basedir'] . '/wpsc/' . $year . '/' . $month . '/' . $term_meta['save_file_name'][0];
								}
								$wpdb->insert(
