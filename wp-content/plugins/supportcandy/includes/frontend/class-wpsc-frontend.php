		...
		public static function load_authentication_screen( $otp_login = true, $additional_links = true ) {
			...
			<div class="wpsc-auth-container">
				<div class="auth-inner-container">
					...
					<form onsubmit="return false;" class="wpsc-login">
						<?php
					...
					<div class="auth-links">
						...
						if ( $page_settings['user-login'] === 'default' ) :
							?>
							<a class="wpsc-link" href="<?php echo esc_url( wp_lostpassword_url() ); ?>"><?php esc_attr_e( 'Forgot your password?', 'supportcandy' ); ?></a>
							<?php
						endif;
						...
						if ( $page_settings['user-registration'] !== 'disable' ) {
							?>
							<a class="wpsc-link" href="<?php echo esc_attr( $registration_url ); ?>"><?php esc_attr_e( 'Register now', 'supportcandy' ); ?></a>
							<?php
						}
						if ( $otp_login && $page_settings['otp-login'] && in_array( 'guest', $gs['allow-create-ticket'] ) ) :
							?>
							<a class="wpsc-link" href="javascript:wpsc_get_guest_sign_in();"><?php esc_attr_e( 'Sign-in using one time password', 'supportcandy' ); ?></a>
							<?php
						endif;

						if ( $additional_links && in_array( 'guest', $gs['allow-create-ticket'] ) ) :
							?>
							<a class="wpsc-link" href="javascript:wpsc_get_guest_ticket_form();"><?php esc_attr_e( 'Create new ticket as guest', 'supportcandy' ); ?></a>
							<?php
						endif;

						if ( $additional_links && $page_settings['open-ticket-page'] ) :
							?>
							<a class="wpsc-link" href="<?php echo esc_url( get_permalink( $page_settings['open-ticket-page'] ) ); ?>"><?php esc_attr_e( 'Open existing ticket using one time password', 'supportcandy' ); ?></a>
							<?php
						endif;
						?>
					</div>
				...
				<script>
					...
					function wpsc_set_default_registration(el) {
						...
						if ( $recaptcha['allow-recaptcha'] === 1 && $recaptcha['recaptcha-version'] == 3 && $recaptcha['recaptcha-site-key'] && $recaptcha['recaptcha-secret-key'] ) {
							?>
							grecaptcha.ready(function() {
								grecaptcha.execute('<?php echo esc_url( $recaptcha['recaptcha-site-key'] ); ?>', {action: 'submit_registration'}).then(function(token) {
									dataform.append('g-recaptcha-response', token);
