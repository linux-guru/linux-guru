...
if ( ! class_exists( 'WPSC_DF_Customer' ) ) :
	...
	final class WPSC_DF_Customer {
		...
		public static function init() {
			...
			// TFF!
			add_action( 'wpsc_create_ticket_data', array( __CLASS__, 'set_create_ticket_data' ), 6, 3 );
		...
		public static function set_create_ticket_data( $data, $custom_fields, $is_my_profile ) {
			...
			if ( $current_user->is_customer && $current_user->customer->email == $email ) {
				$data['customer'] = $current_user->customer->id;
				return $data;
			}
			...
			if ( ! $user && $advanced['register-user-if-not-exist'] ) {
				...
				$customer = ! $customer->id ? WPSC_Customer::get_by_email( $email ) : $customer;

			} elseif ( ! $customer->id && ! $advanced['register-user-if-not-exist'] ) {
