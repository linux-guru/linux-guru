...
if ( ! class_exists( 'WPSC_CF_Email' ) ) :
	...
	final class WPSC_CF_Email {
		...
		public static function print_tff( $cf, $tff ) {
			...
			$val = '';
			if ( $cf->field == 'ticket' ) {
				$val = $cf->is_auto_fill && $cf->default_value ? htmlspecialchars( $cf->default_value[0] ) : '';
			} else {
				$current_user = WPSC_Current_User::$current_user;
				$val          = $current_user->is_customer && $current_user->customer->{$cf->slug} ? htmlspecialchars( $current_user->customer->{$cf->slug} ) : '';
			}
		...
		public static function ticket_search( $sql, $filter, $custom_fields ) {
			...
			if ( isset( $custom_fields[ self::$slug ] ) ) {
				...
				foreach ( $custom_fields[ self::$slug ] as $cf ) {
					...
					if ( ! ( class_exists( $cf->type ) && in_array( $cf->field, WPSC_CF_Settings::$allowed_modules['ticket-filter'] ) ) ) {
						continue;
					}
