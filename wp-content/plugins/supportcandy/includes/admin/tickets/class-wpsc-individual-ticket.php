	...
	final class WPSC_Individual_Ticket {
			...
			if ( self::$view_profile == 'agent' ) {
				...
				if ( self::has_ticket_cap( 'reply' ) || self::is_customer() ) {
					$submit_actions['reply'] = array(
						...
						'callback' => 'wpsc_it_add_reply(' . self::$ticket->id . ', \'' . esc_attr( wp_create_nonce( 'wpsc_it_add_reply' ) ) . '\');',
					);
				}
				if ( self::has_ticket_cap( 'pn' ) ) {
					$submit_actions['private-note'] = array(
						...
						'callback' => 'wpsc_it_add_private_note(' . self::$ticket->id . ', \'' . esc_attr( wp_create_nonce( 'wpsc_it_add_private_note' ) ) . '\');',
					);
				}
		...
		public static function get_reply_section() {
						...
						<div class="submit-container">
							...
							if ( self::$view_profile == 'agent' ) {
								...
								<button 
									...
									class="wpsc-it-editor-submit wpsc-button normal primary" 
									type="button" 
									onclick="wpsc_it_add_reply(<?php echo esc_attr( self::$ticket->id ); ?>, '<?php echo esc_attr( wp_create_nonce( 'wpsc_it_add_reply' ) ); ?>')">
			...
			<script>
				...
				* @param {*} ticket_id 
				*/
				function wpsc_it_add_reply(ticket_id, nonce) {
				 ...
				 * Add private note
				 */
				function wpsc_it_add_private_note(ticket_id, nonce) {
		...
		public static function print_thread( $thread ) {
			...
			$time_title    = $advanced['thread-date-display-as'] == 'date' ? $time_diff_str : $time_date_str;
		...
		public static function add_reply() {
			...
			if ( check_ajax_referer( 'wpsc_it_add_reply', '_ajax_nonce', false ) != 1 ) {
		...
		public static function add_note() {
			...
			if ( check_ajax_referer( 'wpsc_it_add_private_note', '_ajax_nonce', false ) != 1 ) {
				wp_send_json_error( 'Unauthorised request!', 401 );
			}
		 ...
		 * Change assignee
		 *
		 * @param array   $prev - privious customer.
		 * @param array   $new - new customer.
		 * @param integer $customer_id - customer id.
		 * @return void
		 */
		...
		public static function get_duplicate_attachment_id( $attachment_id ) {
			...
			$img_extensions = array( 'png', 'jpeg', 'jpg', 'bmp' );
			...
			// copy to path.
			copy( $attachment->file_path, $filepath );
			$data['is_active'] = 1;
