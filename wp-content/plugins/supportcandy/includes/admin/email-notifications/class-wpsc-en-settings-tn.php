...
if ( ! class_exists( 'WPSC_EN_Settings_TN' ) ) :
	...
	final class WPSC_EN_Settings_TN {
		...
		public static function init() {
			...
			add_action( 'admin_init', array( __CLASS__, 'load_events' ) );
		...
		/**
		 * Load ticket notification events
		 *
		 * @return void
		 */
		public static function load_events() {

			self::$events = apply_filters( 'wpsc_ticket_notification_events', array() );
		}
				...
				<table class="ticket-notification-list wpsc-setting-tbl">
					<thead>
						<tr>
							...
							<th><?php esc_attr_e( 'Event', 'supportcandy' ); ?></th>
						...
						if ( $email_templates ) :
							foreach ( $email_templates as $index => $et ) :
								...
								<tr>
									...
									<td><?php echo esc_attr( self::$events[ $et['event'] ] ); ?></td>
									...
									<td>
										...
										<a href="javascript:wpsc_en_clone_ticket_notification(<?php echo esc_attr( $index ); ?>, '<?php echo esc_attr( wp_create_nonce( 'wpsc_en_get_edit_ticket_notification' ) ); ?>')"><?php esc_attr_e( 'Clone', 'supportcandy' ); ?></a> |
		...
		public static function get_add_ticket_notification() {
			...
			<form action="#" onsubmit="return false;" class="wpsc-frm-add-en">
				...
				<div class="wpsc-input-group">
					<div class="label-container">
						<label for=""><?php esc_attr_e( 'Event', 'supportcandy' ); ?></label>
					</div>
					<select name="event">
						<?php
						foreach ( self::$events as $key => $name ) {
							?>
							<option value="<?php echo esc_attr( $key ); ?>"><?php echo esc_attr( $name ); ?></option>
							<?php
						}
						?>
					</select>
		...
		public static function get_edit_ticket_notification() {
			...
			<form action="#" onsubmit="return false;" class="wpsc-frm-edit-en">
				...
				<div class="wpsc-input-group">
					...
					<input type="text" value="<?php echo esc_attr( self::$events[ $et['event'] ] ); ?>" readonly>
				</div>

				<div class="wpsc-input-group">
					<div class="label-container">
						<label for=""><?php esc_attr_e( 'Status', 'supportcandy' ); ?></label>
					</div>
					<select name="is_enable">
						<option <?php selected( $et['is_enable'], 1 ); ?> value="1"><?php esc_attr_e( 'Enabled', 'supportcandy' ); ?></option>
						<option <?php selected( $et['is_enable'], 0 ); ?> value="0"><?php esc_attr_e( 'Disabled', 'supportcandy' ); ?></option>
					</select>
				</div>
						...
						<div class="wpsc-it-editor-action-container">
							<div class="actions">
								<div class="wpsc-editor-actions">
									<span class="wpsc-link" onclick="wpsc_get_macros()"><?php esc_attr_e( 'Insert Macro' ); ?></span>
								</div>
							</div>
						</div>
					</div>
		...
		/**
		 * Clone ticket notification
		 *
		 * @return void
		 */
		public static function clone_ticket_notification() {
