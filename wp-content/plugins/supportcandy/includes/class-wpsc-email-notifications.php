...
if ( ! class_exists( 'WPSC_Email_Notifications' ) ) :
	...
	class WPSC_Email_Notifications {
		...
		public static function init() {
			...
			add_action( 'wp_ajax_wpsc_send_email_notifications', array( __CLASS__, 'send_emails' ) );
			add_action( 'wp_ajax_nopriv_wpsc_send_email_notifications', array( __CLASS__, 'send_emails' ) );
		}

		/**
		 * Send emails callback after every event from user interface
		 *
		 * @return void
		 */
		public static function send_emails() {

			self::send_background_emails();
			wp_die();
		}
		...
		public function is_valid() {
			...
			$this->body = WPSC_Macros::replace( $body, $this->ticket );
			...
			if ( ! WPSC_Ticket_Conditions::is_valid_conditions( $this->ticket, $this->template['conditions'], $this->template['relation'] ) ) {
				return false;
			}
		...
		public function set_to_addresses() {
			...
			$to = array_diff( $to, $gs['blocked-emails'] );
		...
		public function set_cc_addresses() {
			...
			$cc = array_diff( $cc, $gs['blocked-emails'] );
		...
		public function set_bcc_addresses() {
			...
			$bcc = array_diff( $bcc, $gs['blocked-emails'] );
