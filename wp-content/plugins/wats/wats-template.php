...
function wats_list_tickets_args($args)
{
	...
	{
		$output = __('The frontend ticket listing feature is only available in the premium release. Don\'t hesitate to order the premium release on http://www.ticket-system.net/order-now/','WATS');
	}
...
function wats_ticket_submit_form()
{
	...
	if (WATS_PREMIUM == false)
	{
		$output = __('The frontend submission form feature is only available in the premium release. Don\'t hesitate to order the premium release on http://www.ticket-system.net/order-now/','WATS');
	}
...
function wats_single_ticket_content_filter($content)
{
	...
	if (wats_check_visibility_rights())
	{
		...
		if ($wats_settings['ticket_priority_key_enabled'] == 1 && ($wats_settings['wats_ticket_default_keys'][1]['ftdt'] == 1 || ($wats_settings['wats_ticket_default_keys'][1]['ftdt'] == 2 && current_user_can('administrator'))))
			$output .= '<div id="wats_single_ticket_priority" class="wats_priority_'.esc_attr(get_post_meta($post->ID,'wats_ticket_priority',true)).'"><label class="wats_label">'.__("Current priority : ",'WATS').'</label>'.esc_html(wats_ticket_get_priority($post)).'</div>';
		if ($wats_settings['ticket_status_key_enabled'] == 1 && ($wats_settings['wats_ticket_default_keys'][2]['ftdt'] == 1 || ($wats_settings['wats_ticket_default_keys'][2]['ftdt'] == 2 && current_user_can('administrator'))))
			$output .= '<div id="wats_single_ticket_status" class="wats_status_'.esc_attr(get_post_meta($post->ID,'wats_ticket_status',true)).'"><label class="wats_label">'.__("Current status : ",'WATS').'</label>'.esc_html(wats_ticket_get_status($post)).'</div>';
		if ($wats_settings['ticket_type_key_enabled'] == 1 && ($wats_settings['wats_ticket_default_keys'][0]['ftdt'] == 1 || ($wats_settings['wats_ticket_default_keys'][0]['ftdt'] == 2 && current_user_can('administrator'))))
			$output .= '<div id="wats_single_ticket_type" class="wats_type_'.esc_attr(get_post_meta($post->ID,'wats_ticket_type',true)).'"><label class="wats_label">'.__("Ticket type : ",'WATS').'</label>'.esc_html(wats_ticket_get_type($post)).'</div>';
		if ($wats_settings['ticket_product_key_enabled'] == 1 && ($wats_settings['wats_ticket_default_keys'][3]['ftdt'] == 1 || ($wats_settings['wats_ticket_default_keys'][3]['ftdt'] == 2 && current_user_can('administrator'))))
			$output .= '<div id="wats_single_ticket_product" class="wats_product_'.esc_attr(get_post_meta($post->ID,'wats_ticket_product',true)).'"><label class="wats_label">'.__("Ticket product : ",'WATS').'</label>'.esc_html(wats_ticket_get_product($post)).'</div>';
		...
		if (current_user_can('administrator'))
		{
			...
			if ($ticket_author_name)
				$output .= '<div id="wats_single_ticket_author_name"><label class="wats_label">'.__('Ticket author name : ','WATS').'</label>'.esc_html($ticket_author_name).'</div>';
			$ticket_author_email = get_post_meta($post->ID,'wats_ticket_author_email',true);
			if ($ticket_author_email)
				$output .= '<div id="wats_single_ticket_author_email"><label class="wats_label">'.__('Ticket author email : ','WATS').'</label>'.'<a href="mailto:'.esc_attr($ticket_author_email).'">'.esc_html($ticket_author_email).'</a></div>';
			$ticket_author_url = get_post_meta($post->ID,'wats_ticket_author_url',true);
			if ($ticket_author_url)
				$output .= '<div id="wats_single_ticket_author_url"><label class="wats_label">'.__('Ticket author url : ','WATS').'</label>'.'<a href="'.esc_attr($ticket_author_url).'">'.esc_html($ticket_author_url).'</a></div>';
		}
		$output .= '</div>';
		...
		if ($attachments)
		{
			...
			foreach ($attachments as $attachment)
			{
				...
				$after_content .= '<tr><td><a href="'.esc_attr($url).'">'.esc_html($title).'</a></td><td>'.esc_html($author).'</td><td>'.$attachment->post_date.'</td><td>'.$size.'</td></tr>';
			}
			$after_content .= '</table>';
		}
...
function wats_register_taxonomy()
{
	...
	$taxonomies[] =  'category';
	if (isset($wats_settings['tickets_tagging']) && $wats_settings['tickets_tagging'] == 1)
		$taxonomies[] = 'post_tag';
...
function wats_get_comments_clauses($clauses)
{
	...
	if (!current_user_can('administrator'))
	{
		if ($clauses['fields'] !== '*' && $clauses['fields'] !== 'COUNT(*)')
			$clauses['fields'] = 'wp1.comment_ID';
		else
			$clauses['fields'] = 'COUNT(DISTINCT wp1.comment_ID)';
		
		$clauses['where'] = str_replace('comment_author_email','wp1.comment_author_email',$clauses['where']);		
		$clauses['where'] = str_replace('comment_approved','wp1.comment_approved',$clauses['where']);
		$clauses['where'] = str_replace($wpdb->comments.'.comment_ID','wp1.comment_ID',$clauses['where']);
		...
		$clauses['where'] = str_replace($wpdb->comments.'.comment_date_gmt','wp1.comment_date_gmt',$clauses['where']);
